...
export default function BrandesDitalse() {
  let { addProtuctToCart } = useContext(CartContext);
  const { id } = useParams();
  const [products, setProducts] = useState([]);
  const [isLoading, setIsLoading] = useState(false);

  async function getProductsByBrandes() {
    try {
      setIsLoading(true);
  ...
  return (
    <>
      <div className="h-68 text-center flex justify-center items-center  text-gray-500">
        <p>No products found for this brandes.</p>
      </div>
      {/*     <div className="p-4">
      ...
      {products.length === 0 ? (
        <p className="text-center text-gray-500">
          No products found for this brandes.
        </p>
      ) : (
        <>
